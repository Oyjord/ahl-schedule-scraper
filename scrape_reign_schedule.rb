  ...
  begin
    ...
    lines.each do |line|
      ...
      if match
        ...
        # Fallback for goals with no comma before time
        ...
        if loose_match
          ...
          # Ultra-loose fallback for scorer + time + (SH) with extra spacing
          ultra_loose = line.match(/(?:\d+(?:st|nd|rd|th)?\s*Period-)?\d+,\s*(#{Regexp.escape(home_team)}|#{Regexp.escape(away_team)}),\s*([A-Za-z\s\-']+\d)\s+(\d{1,2}:\d{2})\s*(SH\|PP\|EN)/)
